...
var forms = function forms() {
  ...
  var sendFormData = function sendFormData(url, formData) {
    var res;
    return regeneratorRuntime.async(function sendFormData$(_context) {
      while (1) {
  ...
  var removeMessageDiv = function removeMessageDiv() {
    messageDiv.remove();
  };

  var clearAllInputs = function clearAllInputs() {
...
var tabs = function tabs(tabsButtonsWrapperSelector, tabsButtonSelector, tabContentSelector, activeClass) {
  ...
  hideAllTabs();
  showTab();
  tabButtonsWrapper.addEventListener('click', function (e) {
    var target = e.target;

    if (target.classList.contains(tabsButtonSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabsButtonSelector.replace(/\./, ''))) {
      hideAllTabs();
